...
class decision_maker(Node):
    def __init__(
        ...
        self.publisher = self.create_publisher(
            publisher_msg, publishing_topic, qos_profile=qos_publisher
        )
    ...
    def timerCallback(self):
        ...
        if type(self.goal) == list:
            reached_goal = (
                True
                if calculate_linear_error(self.localizer.getPose(), self.goal[-1])
                < self.reachThreshold
                else False
            )
        else:
            reached_goal = (
                True
                if calculate_linear_error(self.localizer.getPose(), self.goal)
                < self.reachThreshold
                else False
            )

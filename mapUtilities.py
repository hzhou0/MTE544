...
class mapManipulator(Node):
    ...
    def cell_2_position(self, pix: list[int]|tuple[int,int]):
        i, j = pix
        return (
            self.o_x + i * self.getResolution(),
            (self.height - j) * self.getResolution() + self.o_y,
        )

    def position_2_cell(self, pos) -> tuple[int, int]:
        x, y = pos
        return floor((-self.o_x + x) / self.getResolution()), -floor(
            -self.height + (-self.o_y + y) / self.getResolution()
        )
    ...
    def make_likelihood_field(self):
        ...
        occupied_points = [
            self.cell_2_position([i, j]) for i, j in zip(indices[0], indices[1])
        ]
        all_positions = [
            self.cell_2_position([i, j])
        ...
        likelihood_field_img = np.array(
            255 - 255 * probabilities.reshape(image_array.shape), dtype=np.int32
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--map",
        type=str,
        default="./test_map/map.yaml",
        help="the absolute path to argument",
    )
    parser.add_argument("--std", type=float, help="the std", default=0.1)

    args = parser.parse_args()

    MAP_UTILITIES = mapManipulator(args.map, args.std)
    likelihood=MAP_UTILITIES.make_likelihood_field()
    MAP_UTILITIES.plot_pgm_image(likelihood)
    pass
